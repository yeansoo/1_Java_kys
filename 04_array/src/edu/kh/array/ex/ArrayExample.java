...
public class ArrayExample {
	...
	public void ex6() {
		...
		String[] menuArr= {"김밥+라면","KFC","맘스터치","서브웨이","투썸","백반","순대국","곰탕"};
	...
	public void ex7() {
		...
		// 인원 수를 입력받아 그 크기만큼의 정수 배열 선언 및 할당
		// 인원 수 만큼 점수를 입력받아 
		// 합계, 평균, 최고점, 최저점을 출력
		...
		// 배열의 첫 번째 인덱스 값을 가지고 최고/최저의 비교 기준으로 삼음
		...
		System.out.println("최고점 : "+ Arrays.stream(scoreArr).max().getAsInt());
		System.out.println("최저점 : "+ Arrays.stream(scoreArr).min().getAsInt());
	...
	public void ex8() {
		...
		// 입력 받은 정수가 배열에 존재하면 몇 번 인덱스에 존재하는지 출력해라
		// 단, 없다면 "존재하지 않습니다." 출력
		...
		boolean flag=true;
		// for문에 종료된 후에도 true : 존재하지 않는다
		// for문에 종료된 후에도 false : 존재한다
		
		int input=sc.nextInt();
		
		// arr 배열 순차 접근(반복 접근)
		for(int i=0;i<arr.length;i++) {
			if(input==arr[i]) { //  arr[i] 값이 입력 받은 값과 같다면
	...
	public void createLottoNumber_ex() {
	...
	public void ex9() {
		...
		// 얕은 복사
		// - 참조하는 주소만을 복사하여 
		// 서로 다른 참조 ㅅ변수가 하나의 배열(또는 객체)를 참조하게하는 복사
		
		// 특징 : 하나의 배열을 참조하기 때문에 값을 공유하게 된다. 
		
		int[] arr= {99, 70, 80, 90, 40};
		...
		// 복사본의 값을 변경 -> 원본도 값이 변한 == 값을 공유(얕은 복사 특징)
		copyarr[2]=10000;
	...
	public void ex10() {
		...
		// 1) for문을 이용하는 방법(index가 동일하다는 특징을 사용)
//		for(int i=0;i<arr.length;i++) {
//			copyArr[i]=arr[i];
//		}
		
		
		// 2. System.arraycopy(원본배열, 원본 복사 시작인덱스, 
		//						복사배열, 복사배열의 삽입 시작인덱스, 복사길이)
	...
	public void ex11() {
		...
		// arr1==null -> false == arr1 참조변수가 무언가를 참조하고 있다
		...
		int[] arr2=null;
		// 배열 참조변수 선언 및 null 초기화
		// == 값은 있으나 참조하는게 없다라는 뜻
		
		System.out.println(arr2);
		
		// arr2가 참조하는 배열이 없을 때
		// 새로운 배열을 생성하여 그 시작주소를 arr2에 대입
		if(arr2==null) {
